...
class Ninedraft:
    ...
    def __init__(self, master):
        ...
        sbar.add_command(label="Exit", command=self.quit_conform)

        fbar.add_cascade(label='File', menu=sbar)
        self._master.config(menu=fbar)
        self._master.protocol("WM_DELETE_WINDOW", self.quit_conform)
    ...
    def quit_conform(self):
        result = simpledialog.messagebox.askyesno(title='quit confirm', message='Do you really want to quit?')
        if result:
            self._master.quit()

    def restart_confirm(self):
        result = simpledialog.messagebox.askyesno(title='restart confirm', message='Do you really want to restart?')
        if result:
